...
def fabolas(exp_config):
    ...
    ## zhenjie change
    y_for_train = data.get_data(torch.from_numpy(X[:,:-1]), torch.tensor(1.))
    y_high_for_train = max(y_for_train)
    ##

    recording = {"cost": [model_cost_exp.compute_model_cost_fabolas(X[:, 0], X[:, -1]).item()],
                 "incumbents": [y_high_for_train.item()],
                 "operation_time": [0]}
    ...
    while flag:
        ...
        ## zhenjie change
        y_high_for_train_1 = data.get_data(torch.from_numpy(X[-1,:-1]), torch.tensor(1.))
        y_high_for_train = max(y_high_for_train, y_high_for_train_1)
        ##
        
        recording["incumbents"].append(y_high_for_train.item())
