...
class PES:
    def __init__(self, mean_func, variance_func, X_search_range, M):
        """
        Initialize the Predictive Entropy Search (PES) computation class.

        Args:
            mean_func: Function to compute the mean of the Gaussian process.
            variance_func: Function to compute the variance of the Gaussian process.
            X_search_range: The range of each variables in X.
            M: M samples to be used to compute the acq functions.
        """
        self.mean_func = mean_func
        self.variance_func = variance_func
        self.X_search_range = X_search_range
        self.X_dims = len(X_search_range)
        self.M = M     
    
    def var_condition_global_maximizer(self, sample_x, x_star):
        m_list = self.mean_func(sample_x, x_star)
        v_list = self.variance_func(sample_x, x_star)
        ind = torch.tensor(-1, 1)
        v = torch.sum(v_list)
        m = m_list[1] - m_list[0]
        alpha = m / torch.sqrt(v)
        beta = norm.cdf(alpha) / norm.pdf(alpha)
        v_star = v_list[0] - 1/v * beta * (beta + alpha) * torch.pow(v_list[0], 2)
        return v_star

    def forward(self, X, f_best):

        v_x = self.variance_func(X)
        X_dimension = X.shape[0]
        H_1 = 0.5 * torch.log(2 * PI * torch.exp(v_x + EPS))

        pes = []
        for i in range(self.M):
            sample_x = torch.concat([torch.rand(1)* (self.X_search_range[j][1] - self.X_search_range[j][0]) + self.X_search_range[j][0] for j in range(X_dimension)])
            x_star = f_best

            v_x_star = self.var_condition_global_maximizer(sample_x, x_star)

            H_2 = 0.5 * torch.log(2 * PI * torch.exp(v_x_star + EPS))
            pes.append(H_1 - H_2)

        return torch.mean(torch.tensor(pes))
