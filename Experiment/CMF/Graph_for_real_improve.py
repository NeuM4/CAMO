...
seed_dic = {'VibratePlate': [range(0, 30)], 
            'HeatedBlock': [range(0, 30)]}
...
for kk in range(2):
    ...
    for methods_name in cmf_methods_name_list:
        ...
        for seed in seed_dic[data_name]:
            path = os.path.join(sys.path[-1], 'ICLR_exp2', 'CMF', 'Exp_results',Exp_marker,
...
plt.tight_layout()
plt.savefig(os.path.join(sys.path[-1], 'ICLR_exp', 'CMF', 'Graphs') + '/' + 'HeatedBlock_' + cost_name +'_SR_together.pdf', bbox_inches='tight')