...
# The paper provides results for 20 seeds, and 30 seeds were created here
...
for kk in range(2):
    ...
    for methods_name in cmf_methods_name_list:
        ...
        ll = axs[0, kk].plot(cost_x, mean, ls=Dic[methods_name][-1], color=Dic[methods_name][0],
...
for kk in range(2):
    ...
    for methods_name in cmf_methods_name_list:
        ...
        for seed in seed_dic[data_name]:
            path = os.path.join(sys.path[-1], 'Experiment', 'CMF', 'Exp_results',
                                data_name, cost_name, methods_name + '_seed_' + str(seed) + '.csv')
            data = pd.DataFrame(pd.read_csv(path))
            oper_time = data['operation_time'].to_numpy()
            time = np.cumsum(oper_time)
            SR = data['SR'].to_numpy()
            inter = interp1d(time, SR, kind='linear', fill_value=np.nan, bounds_error=False)
            time_collection.append(time)
            inter_collection.append(inter)

        time_x = np.unique(np.concatenate(time_collection))
        SR_new = [inter_collection[i](time_x) for i in range(len(inter_collection))]
        SR_new = np.array(SR_new)
        mean = np.mean(SR_new, axis=0)
        var = np.std(SR_new, axis=0)
        

        ll = axs[1, kk].plot(time_x, mean, ls=Dic[methods_name][-1], color=Dic[methods_name][0],
                    label=Dic[methods_name][2],
                    marker=Dic[methods_name][1], markersize=12,markevery=60)
        axs[1, kk].fill_between(time_x,
                        mean - 0.96 * var,
                        mean + 0.96 * var,
                        alpha=0.05, color=Dic[methods_name][0])
    ...
    for methods_name in baseline_list:
        ...
        for seed in seed_dic[data_name]:
            ...
            data = pd.DataFrame(pd.read_csv(path))
            if methods_name == 'fabolas':
                time_tem = data['operation_time'].to_numpy()
                time = time_tem
            else:
                cost_tem = data['time'].to_numpy()
                time = np.cumsum(time_tem)
                
            if methods_name == 'smac' and data_name == 'VibratePlate':
                time = time + 106  # VibratePlate optimize start from 106s
            elif methods_name == 'smac' and data_name == 'HeatedBlock':
                time = time + 50   # HeatedBlock optimize start from 50s
...
leg = fig.legend(lines, labels, loc='upper center', bbox_to_anchor=(0.5, 1.11), fancybox=True, mode='normal', ncol=4, markerscale = 1.5, fontsize=25)
...
plt.tight_layout()
plt.savefig(os.path.join(sys.path[-1], 'Experiment', 'CMF', 'Graphs') + '/' + 'CMF_real_data.png', bbox_inches='tight')