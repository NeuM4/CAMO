...
seed_dict = {'Branin':[0,1,2,3,5],'Currin':[0,1,3,4,5]}
...
for kk in range(2):
    ...
    for methods_name in methods_name_list:
        ...
        for seed in seed_dict[data_name]:
            path = os.path.join(sys.path[-1], 'Experiment', 'DMF', 'Exp_results', Exp_marker,
    ...
    for methods_name in our_method_list:
        ...
        inter_collection = []
        for seed in seed_dict[data_name]:
            path = os.path.join(sys.path[-1], 'Experiment', 'DMF', 'Exp_results','improve_our_con-dis',
...
leg = fig.legend(lines, labels, loc='upper center', bbox_to_anchor=(0.51, 1.32), fancybox=True, mode='normal', ncol=4, markerscale = 1.3, fontsize=25)
...
plt.tight_layout()
plt.savefig(os.path.join(sys.path[-1], 'Experiment', 'DMF', 'Graphs') + '/' +'Con-dis-' + Exp_marker + '_SR_DNN.png', bbox_inches='tight')