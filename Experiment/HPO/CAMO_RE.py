        ...
        try:
            ...
            if i == 0:
                ...
                for k in range(exp_config['data_num']):
                    mse = data.get_mse(x_train[k], eval=True)
                    if mse > best_mse:
                        best_mse = mse
                recording['wallclocktime'].append(time.time() - time1)
                recording['mse'].append(mse.item())
            ...
            mse = data.get_mse(new_x[0], eval=True)
            if mse < best_mse:
                best_mse = mse
            logging.info(f"Optimization finished {i} times. New x: {new_x}, New y: {new_y}, Best acc: {best_mse.item()}")
            recording['wallclocktime'].append(time.time() - time1)
            recording['mse'].append((best_mse).item())
            i += 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CAMO-HPO-RE")
    parser.add_argument("--start_seed", type=int, default=0)
    parser.add_argument("--data_name", type=str, default="RF_Regressor", choices=list(Data_list.keys()), help="Name of the data model to use")
    parser.add_argument("--MF_model", type=str, default="CMF_CAR", choices=list(MF_model_list.keys()), help="Model to use for multi-fidelity")
