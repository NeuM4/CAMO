...
def train_CAR(CARmodel, data_manager, max_iter=1000, lr_init=1e-1, normal = True, debugger = None):
    ...
    for i_fidelity in range(CARmodel.fidelity_num):
        ...
        if i_fidelity == 0:
            ...
            if CARmodel.if_nonsubset:
                with torch.no_grad():
                    subset_x, y_low, y_high = data_manager.get_nonsubset_fill_data(CARmodel, i_fidelity-1, i_fidelity, normal = normal)
            else:
...
if __name__ == "__main__":
    ...
    train_CAR(CAR,fidelity_manager, max_iter=200, lr_init=1e-2, debugger = None)
