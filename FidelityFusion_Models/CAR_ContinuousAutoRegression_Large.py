...
class fidelity_kernel_MC(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        X1 = x1[:, 0].reshape(-1, 1)
        X2 = x2[:, 0].reshape(-1, 1)
        fidelity_indicator_1 = x1[:, 1].reshape(-1, 1).double()
        fidelity_indicator_2 = x2[:, 1].reshape(-1, 1).double()

        N = 100
        torch.manual_seed(self.seed)
        # t1 ????
        t1 = torch.rand(N).double().reshape(N, 1) # 这块需要用来调整z选点的范围
        t2 = torch.rand(N).double().reshape(N, 1)
...
class ContinuousAutoRegression_large(nn.Module):
    ...
    def __init__(self, input_dim, kernel_x, b_init=1.0):
        ...
        kernel_full = fidelity_kernel_MC(input_dim, kernel_x, self.b)
    ...
    def forward(self, data_manager, x_test):
        ...
        x_train, y_train = data_manager.get_data(0)
        y_pred, cov_pred = self.cigp(x_train,y_train,x_test)

        # return the prediction
        return y_pred, cov_pred
    
def train_CAR_large(CARmodel, data_manager,max_iter=1000,lr_init=1e-1):
    # x_train = []
    # y_train = []
    # fidelity_indicator = []
    ...
    x_train, y_train = data_manager.get_data(0, normal = False)

    optimizer = torch.optim.Adam(CARmodel.parameters(), lr=lr_init)
    for i in range(max_iter):
        optimizer.zero_grad()
        loss = CARmodel.cigp.negative_log_likelihood(x_train, y_train)
        loss.backward()
        optimizer.step()
...
if __name__ == "__main__":
    ...
    x_low = torch.cat((x_low, torch.zeros(x_low.shape[0]).reshape(-1,1)), 1)
    x_high1 = torch.cat((x_high1, torch.ones(x_high1.shape[0]).reshape(-1,1)), 1)
    x_high2 = torch.cat((x_high2, 2*torch.ones(x_high2.shape[0]).reshape(-1,1)), 1)
    x_test = torch.cat((x_test, 2*torch.ones(x_test.shape[0]).reshape(-1,1)), 1)
    # y_low = torch.cat((y_low, torch.zeros(y_low.shape[0]).reshape(-1,1)), 1)
    # y_high1 = torch.cat((y_high1, torch.ones(y_high1.shape[0]).reshape(-1,1)), 1)
    # y_high2 = torch.cat((y_high2, 2*torch.ones(y_high2.shape[0]).reshape(-1,1)), 1)
    ...
    CAR = ContinuousAutoRegression_large(input_dim=x.shape[1]-1, kernel_x=kernel_x, b_init=1.0)

    train_CAR_large(CAR,fidelity_manager, max_iter=100, lr_init=1e-2)
