...
class CMF_CAR_dkl_s(nn.Module):
    ...
    def forward(self, data_manager, x_test,fidelity_indicator = None, normal = False):
        ...
        x_test_1 = x_test[:,:-1]
        x_test_s = x_test[:,-1]
        x_test_spro = self.FeatureExtractor(x_test_s.view(-1,1).double()).view(-1)
        x_test_pro = torch.cat((x_test_1, x_test_spro.view(-1,1)), 1)
        
        x_train_1 = x_train[:,:-1]
        x_train_s = x_train[:,-1]
        x_train_spro = self.FeatureExtractor(x_train_s.view(-1,1).double()).view(-1)
...
def train_CMFCAR_dkl_s(CARmodel, data_manager,max_iter=1000,lr_init=1e-1, normal = False):
    '''
    # x_train = []
    # y_train = []
    # fidelity_indicator = []
    ...
    for i in range(max_iter):
        ...
        x_train_1 = x_train[:,:-1]
        x_s = x_train[:,-1]
        x_s_process = CARmodel.FeatureExtractor(x_s.view(-1,1).double()).view(-1)
...
if __name__ == "__main__":
    ...
    CAR = CMF_CAR_dkl_s(kernel_x=kernel_x, b_init=1.0)

    train_CMFCAR_dkl_s(CAR, fidelity_manager, max_iter=200, lr_init=1e-2)
