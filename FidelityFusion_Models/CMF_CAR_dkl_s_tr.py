...
class CMF_CAR_dkl_s_tr(nn.Module):
    # initialize the model
    def __init__(self, kernel_x, b_init=1.0, hidden_dim = 4):
        super().__init__()
        # self.fidelity_num = fidelity_num
        self.b = torch.nn.Parameter(torch.tensor(b_init))
        
        encoder_layer = nn.TransformerEncoderLayer(d_model=1, nhead=1, dim_feedforward=4, dropout=0.1)
        self.FeatureExtractor = nn.TransformerEncoder(encoder_layer, num_layers=6)
    ...
    def forward(self, data_manager, x_test,fidelity_indicator = None, normal = False):
        ...
        x_test_s = x_test_s.unsqueeze(0)
        te_s = self.FeatureExtractor(x_test_s.double())
        x_test_spro = self.final_layer(te_s.mean(dim=0))
        x_test_pro = torch.cat((x_test_1, x_test_spro.view(-1,1)), 1)
        
        x_train_1 = x_train[:,:-1]
        x_train_s = x_train[:,-1]
        x_train_s = x_train_s.reshape(-1,1)
        x_train_s = x_train_s.unsqueeze(0)
        tr_s = self.FeatureExtractor(x_train_s.double())
        x_train_spro = self.final_layer(tr_s.mean(dim=0))
        x_train_pro = torch.cat((x_train_1, x_train_spro.view(-1,1)), 1)
        
        y_pred, cov_pred = self.cigp(x_train_pro,y_train.double(),x_test_pro)

        # return the prediction
        return y_pred, cov_pred
    
def train_CMFCAR_dkl_s_tr(CARmodel, data_manager,max_iter=1000,lr_init=1e-1, normal = False):
    '''
    # x_train = []
    # y_train = []
    # fidelity_indicator = []
    ...
    for i in range(max_iter):
        ...
        x_s = x_s.reshape(-1,1)
        x_s = x_s.unsqueeze(0)
        tr_s = CARmodel.FeatureExtractor(x_s.double())
        x_s_process = CARmodel.final_layer(tr_s.mean(dim=0))
        x_pro = torch.cat((x_train_1, x_s_process.view(-1,1)), 1)
        loss = CARmodel.cigp.negative_log_likelihood(x_pro, y_train.double())
        loss.backward()
        optimizer.step()
...
if __name__ == "__main__":
    ...
    CAR = CMF_CAR_dkl_s_tr(kernel_x=kernel_x, b_init=1.0)

    train_CMFCAR_dkl_s_tr(CAR, fidelity_manager, max_iter=200, lr_init=1e-2)
