...
class CMF_CAR_dkl_tr(nn.Module):
    ...
    def __init__(self, input_dim, kernel_x, b_init=1.0):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=input_dim, nhead=1, dim_feedforward= input_dim*2, dropout=0.1)
    ...
    def forward(self, data_manager, x_test,fidelity_indicator = None, normal = False):
        ...
        te_s = self.FeatureExtractor(x_test.double())
        x_test_pro = self.final_layer(te_s.mean(dim=0))
        
        x_train = x_train.unsqueeze(0)
        tr_s = self.FeatureExtractor(x_train.double())
        x_train_pro = self.final_layer(tr_s.mean(dim=0))
...
def train_CMFCAR_dkl_tr(CARmodel, data_manager,max_iter=1000,lr_init=1e-1, normal = False):
    ...
    for i in range(max_iter):
        optimizer.zero_grad()
        x_train1 = x_train.unsqueeze(0)
        tr_s = CARmodel.FeatureExtractor(x_train1.double())
        x_pro = CARmodel.final_layer(tr_s.mean(dim=0))
...
if __name__ == "__main__":
    ...
    CAR = CMF_CAR_dkl_tr(input_dim = x.shape[1], kernel_x=kernel_x, b_init=1.0)

    train_CMFCAR_dkl_tr(CAR, fidelity_manager, max_iter=200, lr_init=1e-2)
    ...
    # plt.show()
    plt.savefig('CMF_CAR_dkl_tr.png') 
