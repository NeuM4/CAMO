...
class cigp_dkl(nn.Module):
    ...
    def forward(self, data_manager, x_test, fidelity_indicator = None, normal = False):
        ...
        x_train1 = self.dkl(x_train.double())
        x_test1 = self.dkl(x_test.double())
        ...
        # option 1
        mean = kx.t() @ torch.cholesky_solve(y_train.double(), L)  # torch.linalg.cholesky()
...
def train_GPdkl(GPmodel, data_manager, max_iter=100, lr_init=1e-1, normal = False):
    ...
    for i in range(max_iter):
        ...
        xtr = GPmodel.dkl(xtr.double())
        loss = GPmodel.negative_log_likelihood(xtr, ytr.double())
        loss.backward()
        optimizer.step()
