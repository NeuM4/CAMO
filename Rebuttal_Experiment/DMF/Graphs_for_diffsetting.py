...
Dic = {
       ...
       'ResGP_cfKG': ['#00CCFF', "X", "ResGP-cfKG", "solid"],
        ...
        'GP_UCBNorm_res': ['blue', "^", "GP_UCB_norm", "solid"],
        'GP_cfKGNorm_res': ['blue', "X", "GP_cfKG_norm", "solid"],
        'CMF_CAR_dkl_UCBNorm_res': ['green', "^", "CMF_CAR_UCB_norm", "dashed"],
        'CMF_CAR_dkl_cfKGNorm_res': ['green', "X", "CMF_CAR_cfKG_norm", "dashed"],
        'GP_UCBRebuttal_all3_improve_acq': ['red', "^", "GP_UCB_unnorm", "solid"],
        'GP_cfKGRebuttal_all3_improve_acq': ['red', "X", "GP_cfKG_unnorm", "solid"],
        'CMF_CAR_dkl_UCBRebuttal_all3_improve_acq': ['orange', "^", "CMF_CAR_UCB_unnorm", "dashed"],
        'CMF_CAR_dkl_cfKGRebuttal_all3_improve_acq': ['orange', "X", "CMF_CAR_cfKG_unnorm", "dashed"],
...
cost_lim_y = {'pow_10': [0.15, 0.6], 'linear': [0, 55], 'log': [0, 55]}
...
data_name = 'Branin' 
Exp_markers = ["Rebuttal_all3_improve_acq","Norm_res"]
...
for kk, cost_name in enumerate(cost_list):
    for methods_name in methods_name_list:
        for Exp_marker in Exp_markers:
            ...
            ax.plot(cost_x, mean + add_dict[data_name], ls=Dic[methods_name+Exp_marker][-1], color=Dic[methods_name+Exp_marker][0],
                    label=Dic[methods_name+Exp_marker][2], marker=Dic[methods_name+Exp_marker][1], markersize=12, markevery=makervery_index)
            ax.fill_between(cost_x, mean + add_dict[data_name] - 0.96 * var,
                            mean + add_dict[data_name] + 0.96 * var, alpha=0.05, color=Dic[methods_name+Exp_marker][0])
...
# plt.show()
plt.savefig(os.path.join(sys.path[-1], 'Rebuttal_Experiment', 'DMF', 'Graphs') + '/' +'No_norm-norm' + data_name + '_SR.pdf', bbox_inches='tight')
