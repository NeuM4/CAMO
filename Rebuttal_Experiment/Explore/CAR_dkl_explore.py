...
def MF_BO_continues(exp_config):
    ...
    while flag:
        ...
        elif exp_config["MF_model"] in ["CMF_CAR","CMF_CAR_dkl_s","CMF_CAR_dkl_s_tr"]:
        ...
        elif exp_config["MF_model"] == "CMF_CAR_dkl_s":
            train_CMFCAR_dkl_s(GP_model, fidelity_manager, max_iter=MF_iterations, lr_init=MF_learning_rate)
        elif exp_config["MF_model"] == "CMF_CAR_dkl_s_tr":
            train_CMFCAR_dkl_s_tr(GP_model, fidelity_manager, max_iter=MF_iterations, lr_init=MF_learning_rate)
        else:
...
if __name__ == '__main__':
    ...
    for seed in range(10):
        for mf_model in ["CMF_CAR_dkl_s_tr"]:
            for acq in ["UCB","cfKG"]:
                try:
                    # Set up logging
                    log_file_path = os.path.join(sys.path[-1], 'Rebuttal_Experiment', 'Explore', 'Exp_results', Exp_marker, data_name, args.cost_type , f'{mf_model}_experiment_{acq}.log')
                    ...
                    path_csv = os.path.join(sys.path[-1], 'Rebuttal_Experiment', 'Explore', 'Exp_results', Exp_marker, data_name, exp_config['cost_type'])
