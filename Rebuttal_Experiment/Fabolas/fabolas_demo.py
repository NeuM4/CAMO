...
def fabolas(exp_config):
    ...
    path_csv = sys.path[-1] + '/Rebuttal_Experiment/CMF/Exp_results/'+exp_config['Exp_marker']+'/' + data_name + '/' + exp_config['cost_type'] +'/'+ 'fabolas_improve'+ '/'
    ...
    if data_name in ['Park', 'Branin', 'Currin']:
        ...
        y_high_for_train = data.get_data(torch.from_numpy(X[:,:-1]), 1)
    ##
...
if __name__ == '__main__':
    Data_list = {'non_linear_sin': non_linear_sin, 'forrester': forrester, 'Park': Park, 'Branin': Branin, 
                 'Currin': Currin, 'borehole':borehole,'booth':booth,"hartmann":hartmann,"bohachevsky":bohachevsky,
