...
class cost_discrete():
    ...
    def compute_model_ConDis_cost(self, dataset):
        C = 0
        for i in range(len(dataset)):
            C += self.compute_cost(i/10) * dataset[i].shape[0]
        return C
    
    def compute_model_ConDisGP_cost(self, dataset):
        C = 0
        for i in range(len(dataset[0])):
            C += self.compute_cost((dataset[0][i][-1]-1)/10)
        return C
